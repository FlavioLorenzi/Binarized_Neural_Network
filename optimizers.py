#Ottima implementazione di uno shift based per AdaMax Optimizer, trovata in rete
...
class ShiftBasedAdaMaxOptimizer(optimizer.Optimizer):
	...
	def __init__(self, learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-8,
		...
		super(ShiftBasedAdaMaxOptimizer, self).__init__(use_locking, name)    #SUPERCLASSE
		...
		#val successivi non ancora assegnati
		self._lr_t = None
		self._beta1_t = None
		self._beta2_t = None
		self._eps_t = None
	...
	#metodo che connette i valori successivi a quelli precedenti creando tensori e quindi una rete connessa
	def _prepare(self):
	...
	#passare esplicitamente la lista delle variabili
	def _create_slots(self, var_list):
		first_var = min(var_list, key=lambda x: x.name)
		self._create_non_slot_variable(initial_value=self.beta1, name="beta1_power",  colocate_with=first_var)
		
		for v in var_list:
			self._zeros_slot(v, "m", self._name)  ####### credo sia errore : ci va m
			self._zeros_slot(v, "v", self._name)
	

	#ottieni il tensor-graph della variabile beta contenuta nello slot
	...
	def _apply_dense(self, grad, var):
		...
		#assegnazione dtype
		beta1_power = math_ops.cast(_beta1_power, grad.dtype.base_dtype) #cast accomuna e fonde due type
		...
		#USO FORMULE PAPER
		#NON DOVREBBE ESSERE IL CONTRARIO (m scambiato con v) ? ? ? ? 
		...
		lr_c = ap2(lr / (1 - beta1_power))  #Primo termine formula
		g_t = v_t / ap2(m_t)     #Secondo termine formula
		var_update = state_ops.assign_sub(var, lr_c * g_t)  #assegna la moltiplicazione a var (aggiornandola)

		return control_flow_ops.group(*[var_update, m_t, v_t])   #return aggiornamento completo gruppo variabili
